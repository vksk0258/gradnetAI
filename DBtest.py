...
for page in response_iterator:
    for content in page['Contents']:
        s3_list.append(content['Key'])

for i in range(1,len(s3_list)):
    file_path = './input/'+s3_list[i][5:]
    bucket = 'grandnet'
    key = s3_list[i]
...
test = pymysql.connect(host='localhost', user='root', password='vlsl0258', db='test', charset='utf8')
...
for i in range(0,len(doc)):
    ...
    path = destination_path + name[0:-4] + ".txt"
    ...
    if result[0] == 0 :
        msg = name+"은(는) "+type +" 맞습니다."
        print(msg)
    else:
        msg = name+"은(는) "+type+" 아닙니다"
        print(msg)
        sql2 = "insert into issue(name,error) values('" + name + "','"+msg+"');"
